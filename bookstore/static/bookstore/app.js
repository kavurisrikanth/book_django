...
function start() {
	...
	$.getJSON(jsonFile, function(myObj) {
		...
		for(i = 0; i < myObj.items.length; i++) {
			...
			var isbn;
			for(var i = 0; i < myObj.items[i].volumeInfo.industryIdentifiers.length; i++)
				if(myObj.items[i].volumeInfo.industryIdentifiers[i].type === 'ISBN_13')
					isbn = myObj.items[i].volumeInfo.industryIdentifiers[i].identifier;
			
			var bookTitleTag = document.createElement('div');
			bookTitleTag.setAttribute('class', 'col-md-6');
			bookTitleTag.innerHTML = '<a href=' + '/viewbook?isbn=' + isbn + '>' + bookTitle + '</a>';
